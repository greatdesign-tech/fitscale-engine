...
import { X, Calendar, Clock, DollarSign, Star, CheckCircle2, Award } from 'lucide-react';
...
export const PtSchedulerModal: React.FC<PtSchedulerModalProps> = ({
  ...
  const price = duration === '30m' ? Math.round(trainer.rate * 0.6) : trainer.rate;

  const handleConfirm = () => {
    setIsConfirming(true);
    setTimeout(() => {
      onSuccess(trainer.name, duration === '30m' ? '30 Minutes' : '60 Minutes', selectedSlot);
        ...
        <div className="absolute inset-0 z-50 flex items-end justify-center bg-black/80 backdrop-blur-xs">
          ...
          >
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-1 text-[11px] text-amber-400">
                    ...
                    <span className="text-slate-400">({trainer.sessionsCompleted} sessions)</span>
                  </div>
                </div>
              </div>
              <button
            ...
            <div className="my-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                <button
                  ...
                  style={{
                    borderColor: duration === '30m' ? config.primaryColor : undefined,
                  }}
                >
                  <div className="text-xs font-bold">30 Minutes</div>
                  <div className="text-[11px] text-slate-400">Express Focus • ${Math.round(trainer.rate * 0.6)}</div>
                </button>
                ...
                <button
                  ...
                  style={{
                    borderColor: duration === '60m' ? config.primaryColor : undefined,
                  }}
                >
                  <div className="text-xs font-bold">60 Minutes</div>
                  <div className="text-[11px] text-slate-400">Full Assessment • ${trainer.rate}</div>
                </button>
              </div>
            </div>
            ...
            <button
              ...
              className="w-full mt-2 py-3 rounded-xl text-xs font-bold transition flex items-center justify-center gap-2 shadow-lg"
              style={{
